...
const listing = await run('Listing Event', async i => (await contract.submitAsync('RecordSupplyChainEvent',{arguments:build(i,'listed','Product','farmer'),endorsingOrganizations:[mspId]})).getStatus());
const shipment = await run('Shipment Event', async i => (await contract.submitAsync('RecordSupplyChainEvent',{arguments:build(i,'shipment_in_transit','Shipment','transporter'),endorsingOrganizations:[mspId]})).getStatus());
